...
- 每个 host-owned child prompt 必须产生该 prompt 之后的新 Pi assistant message；旧的 assistant 尾消息不能被当作当前 turn 的回复。可恢复的无新回复/transport stall 只能在同一 child session 中有界重试。Host-terminal child tool 只有成功结果才能结束当前 tool batch；失败结果必须留在同一 Pi turn 让 child 消费精确错误，禁止因此把完整 assignment 重新派发。
...
- `glossaryCandidates=false` 只关闭新候选的构造与更新，不关闭只读参考：现有 `glossary_candidates.json` 仍可按 assigned-source 命中或精确搜索帮助保持一致。关闭时 child 提交 schema 不得要求或接受新 glossary discovery，Host 必须忽略意外上报，禁止写 candidate、启动术语冲突门、恢复 candidate-derived repair debt 或在最终校验中重建该债务；正式 glossary 的机械校验仍然生效。
- 翻译 child 只能在强制校验工具中结构化上报启用的专名/译名候选和角色事实，并附自己负责行范围内的证据；child 不直接改共享资产。启用 glossary candidate 时，每个 assignment 通过机械校验和只读审阅后，Host 必须立即把术语发现按 source/candidate hash 记为 observed：正式 glossary 优先；同一原词同一译名合并证据并原子写入 provisional glossary candidate；同一原词不同译名不得覆盖或并存于 candidate，必须持久化为 conflict 并关闭整个 batch 的新 assignment 领取门。所有已运行 assignment 到达门后，Host 通过原生 Pi follow-up 唤醒 parent；parent 解决冲突后，Host 在同一 batch 中优先派发并结算精确受影响行修复，全部通过后才恢复原队列。完整 observed 证据继续留在 Host state，character facts 和不能自动决定的非冲突发现仍由 parent 在 completion 阶段统一研究与决策。
...
- 审阅拒绝必须只包含精确失败行、机器码和可直接执行的短修改说明；通过行不产生理由。只涉及目标语言常规标点或排版的差异（例如中文闭引号前补句号）不是翻译安全缺陷，reviewer prompt 与 Host 提交边界都必须过滤，不能因此触发修复循环。相同候选 hash 被复审再次拒绝表示翻译 worker 没有取得进展，必须立即作为不可盲重试的 assignment failure 暴露；候选持续变化但仍未通过时也只允许有界修复，不得重启整块或完整翻译队列反复烧 token。
...
- 术语 priority repair 命中已经接受的 translation alignment range 时，必须复用该 range 的完整写入 ownership，并只重开精确术语行的 review evidence；同一 range 内的术语债务合并为一个 repair assignment，禁止创建与旧 range 重叠的单行 scope 或重新审阅整块。
...
- 失败路径要回到模型或 UI 中形成可恢复状态，不能静默吞掉，也不能假完成。child completion 即使同时建立了 recovery pause，也必须触发一次 parent 报告 turn；pause 只阻止自动重启队列，不能把完成通知作为隐藏 custom message 停放。
