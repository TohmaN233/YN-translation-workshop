...
- 超大审阅页能打开了：不再被 80 MiB 卡死；建议跳到原文，实测从大约 4 秒降到 1–2 秒。
- 大文件翻译、校对更稳，内存不再越跑越高，预扫描也不再卡住主窗口。
- 主窗口能自适应；角色可以给同一人追加多条台词。
- 打开 Agent 或发短问题，不会把审阅页拉回顶部。
- 手机远程改成全屏工作区，Agent 放进工具抽屉。
