...
[English](README.en.md) · [完整教程与技术手册](https://tohman233.github.io/YN-translation-workshop/) · [下载 2.1.0](https://github.com/TohmaN233/YN-translation-workshop/releases/tag/v2.1.0)

## 2.0 → 2.1.0 更新情报

### 2.1.0：超大审阅文件

- 取消行审阅 HTML 的 80 MiB 固定拒绝门槛；确实需要完整正文时改为流式提取内嵌数据。
- 校对建议跳转正文改为只解析路由并导航，不再为一次跳转读取、合并和回写整份 sidecar，也不会经 IPC 复制全部正文。实测 53 万行页面在已加载后由约 3.6–3.9 秒降至约 0.9–2.2 秒。
- 已有 proposal HTML 会从 v11 自动升级到 v12，无需重新生成。首次打开超大单体 HTML 仍受文件解析和 Chromium 渲染速度影响；本次优化针对页面已加载后的跳转。

### 2.0.9：大文件稳定性

- 修复连续分块翻译、校对和精确搜索把整份输入字符串留在历史中的问题，降低长任务不断增长的内存占用。
- 校对全文预扫描移至后台 Worker，并复用已验证的扫描结果；大文件扫描期间主窗口不再被同步计算长期占用。

### 2.0.6 – 2.0.8：主界面与远程操作

- 主窗口改为自适应布局；资产表单更易读，Help 补充术语表和角色表输入示例。
- 角色表可选择已有角色，并为同一角色追加多条台词源词与对应译法，同时兼容旧版 `character_bible.md`。
- 手机远程页改为全屏工作区，Agent 收入工具抽屉；打开 Agent 或发送短问题不再把行审阅页拉回顶部。

### 2.0.1 – 2.0.5：工作流可靠性与 Provider

- 校对与 EPUB 始终遵循页面手选的权威译文绑定；翻译完成后，未手选的页面会跟随正式 `translated` 产物，不再误读临时快照。
- 新增 **Grok (OAuth)**、按模型适配的思考等级和长推理等待；授权过期时刷新并恢复当前工作，而不是中断整批任务。
- 翻译复审耗尽后把精确失败行交回主 Agent，校对 findings 由 Host 按当前绑定行校正；已通过结果不会被后续空写入或失败项清空。
- 术语长名覆盖短名，跨文件术语差异进入最终警告复核；关闭术语候选时不再构造、更新或校验候选资产。
- 批量应用保留人工修改和 TXT 写入状态，恢复命令明确区分翻译与校对，减少跨工作流误恢复。
...
- Windows 安装版：`translation-workshop-Setup-2.1.0-x64.exe`
- Windows 便携版：`translation-workshop-Portable-2.1.0-x64.exe`
- 校验文件：`SHA256SUMS.txt`
