...
HTML 审校界面优先读取 JSON，如果 JSON 不存在，再 fallback 到旧 Markdown 解析逻辑，生成的 HTML 才是给人看的。（另外目前有「全部分类」按钮无效的 bug，Issue 6 验收时需修复。）
...
| Issue | 名称 | 优先级 | 主要 Phase |
|------:|------|--------|------------|
| 1 | 统一翻译协议 | P0 | Phase 0 |
| 2 | AgentProvider 抽象 | P0 | Phase 1 |
| 3 | Agent OS 式聊天入口 | P0 | Phase 1 |
| 4 | AgentJob Runtime | P0 | Phase 1 |
| 5 | Translation Validator | P0 | Phase 0–1 |
| 6 | 审校 JSON schema + HTML 读取 | P1 | Phase 1–2 |
| 7 | runtime 级 Subagent Orchestrator | P1 | Phase 3 |
| 8 | API Agent Provider | P1 | Phase 2 |
| 9 | Workflow Templates | P1 | Phase 2 |
| 11 | 术语表 / 角色表 / 风格指南资产化 | P1 | Phase 3 |
| 12 | revision-based patch conflict | P1 | Phase 3 |
| 13 | 拆分 Electron main process | P2 | Phase 4（可提前部分落地） |
| 14 | parser / schema / validator 回归测试 | P2 | Phase 4（可提前部分落地） |

说明：

- **没有 Issue 10**。旧稿 Phase 3 误把 Issue 11 写成 Issue 10，已更正。
- **初翻不走 Patch Review**（见 §5.4）；Patch Review 仅用于**校对** findings / fix proposals（Issue 6 + 现有 proposal-review HTML）。
- **Issue 5（Validator）** 与 **Issue 4（Job Runtime）** 解耦：Validator 可先独立交付（当前 MVP 已部分完成）。
...
Codex / Claude / API 只作为 adapter（由 `scripts/render-skills.mjs` 等从协议渲染 thin shell）。
...
### 当前进度（agent-workbench-evolution，2026-06）

| 项 | 状态 |
|----|------|
| Issue 1 统一协议 + schema 草案 | ✅ `translation-protocol/`、skill 薄壳渲染 |
| Issue 5 Validator（blocking + warning） | ✅ `src/shared/validation/translationValidator.ts` |
| §5.4 产物发现 + 导入草稿 | ✅ artifact 卡片、不写最终 TXT |
| Issue 13 / 14 部分 | 🟡 `src/main/agent/`、`tests/` 已起步 |
| Issue 2–4 AgentProvider / Chat / Job Runtime | 🟡 Provider + Chat 面板 + Job 持久化已起步 |
| Issue 6 JSON 审校 | ⏳ schema 已有，Host 解析未接 |
...
### Phase 1：Agent 会话入口 + Job Runtime + Validator + JSON 审校
...
- Issue 5：Translation Validator（可与 Job 并行，MVP 已部分完成）；
- Issue 6：审校 JSON schema + HTML 优先读 JSON（校对 findings → 现有 proposal-review / patch 接受流程）。

**不包含在 Phase 1 的初翻路径：** Patch Review 不作为初翻 TXT 的默认模型（见 §5.4）。初翻 = 候选 TXT → validator → 导入草稿。
...
在持久 Agent 会话里创建 job
  ↓
Agent 生成候选 TXT 或 findings JSON
  ↓
Validator 检查
  ↓
产物卡片 / 审校 HTML 展示
...
- Issue 7：Subagent Orchestrator；
- Issue 11：术语表、角色表、风格指南、决策记录；
- Issue 12：revision-based patch conflict handling。
...
- Issue 13：拆分 main process（Agent 相关模块已可提前拆出）；
- Issue 14：补 parser / schema / validator / prompt 回归测试。
...
实现 Agent 会话 / Job / 产物卡片时，可参考以下项目的**交互与持久化思路**，但**不复制**通用 Agent OS 的全套能力：

| 参考 | 可借鉴 | 明确不做 |
|------|--------|----------|
| [Pi.agent](https://github.com/earendil-works/pi) | 持久 session、事件流、工具调用与产物索引 | 通用 coding agent、无关工具生态 |
| [Apix v2.1](https://github.com/JJJJSTIYYYY/Apix/tree/Version_2.1) | 聊天 + 任务卡片 + 可分离面板 | 浏览器自动化、企业权限、多服务部署 |

本 RFC 的范围仍以 **§2–§3** 为准：翻译工作台是中心；Agent 是协作者；行对行数据、validator、人类审阅队列不可省略。

---

## 12. 可能的目录结构

> **技术栈说明：** 渲染层为 **React 19 + TypeScript**（`src/renderer/`），不是 Vue。下列 Agent UI 组件应为 `.tsx`。
...
translation-protocol/          # 协议 Markdown + JSON schema（单一来源）
  translate.md
  proofread.md
  patch.schema.json
  findings.schema.json
...
src/shared/validation/
  ...
  patchValidator.ts            # 后续

src/main/agent/
  conversationStore.ts
  jobStore.ts
  jobManager.ts
  artifactDiscovery.ts         # 已有
  importCandidate.ts           # 已有
  providers/                   # Issue 2 起
    codexCliProvider.ts
    claudeCliProvider.ts
    apiProvider.ts             # Issue 8

src/main/ipc/
  agentArtifactHandlers.ts     # 已有
  agentRuntimeHandlers.ts      # Issue 4

src/renderer/agent/            # React .tsx
  AgentChatPanel.tsx
  AgentJobCard.tsx
  AgentTimeline.tsx
  AgentArtifactList.tsx

src/renderer/review/           # 现有 proposal-review 可演进
  ProposalReviewApp.tsx
```
...
Initial Translation Job
  ↓
Provider Adapter（CLI / API）
  ↓
候选译文 TXT（如 AI_translation/*.txt）
  ↓
产物发现 + Line-aligned Validator
  ↓
导入为译文草稿（工作台 localStorage edits）
  ↓
人类审阅 / 修改
  ↓
用户显式写入绑定 TXT 或导出
